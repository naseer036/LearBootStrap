...
//timer function for moving ball
void Timer(int v)
{
	ball.left+=Xspeed;
	ball.right+=Xspeed;
...
void Render2(void)
{
	...
	glColor3f(1,0,0);
	drawText("\"touching the ball to the cornor of pad will fetch more points\"",80,440);
	glColor3f(0,1,1);
	drawText("P R E S S N TO BACK",200,500);
	glutSwapBuffers();
}
//menus options
void menu(int num)
{
	...
	else
	{
		switch(num)
		{
			...
			case 11:Setting(0.0,0,0.0,0);break;
		}
	}
	glutPostRediplay();
}
...
//start screen
	void startscreen()
	{
		...
		glLoadIdentity();
		glColor3f(1,1,1);
		drawText("WELCOME TO 2D CRICKET",250,150);
		glColor3f(0,1,0);
...
//last screen
	...
	drawText("***PRESS\"e\" TO EXIT THE GAME***",160,500);
	glutSwapBuffers();
}
int pause11()
{
...
void keyboard(unsigned char key,int x,int y)
{
	switch(key)
	{
		...
		case 'c':rend(ball);delta=1;pcResult=0;playerResult=0;game=0;renderr1();break;
		case 'a':rend(ball);delta=1;pcResult=0;playerResult=0;game=1;renderr1();break;
	}
}
